...
'''
Copyright 2015 ACRCloud Recognizer v1.0.0.post1
...
class ACRCloudRecognizer:
    ...
    async def do_recogize(self, host, query_data, query_type, access_key, access_secret, timeout=5):
        http_method = "POST"
    ...
    async def recognize(self, wav_audio_buffer):
        res = ''
        try:
            query_data = {}
    ...
    async def recognize_by_file(self, file_path, start_seconds, rec_length=10):
        res = ''
        try:
            query_data = {}
            if self.recognize_type == ACRCloudRecognizeType.ACR_OPT_REC_AUDIO or self.recognize_type == ACRCloudRecognizeType.ACR_OPT_REC_BOTH:
    ...
    async def recognize_by_filebuffer(self, file_buffer, start_seconds, rec_length=10):
        res = ''
        try:
            query_data = {}
            if self.recognize_type == ACRCloudRecognizeType.ACR_OPT_REC_AUDIO or self.recognize_type == ACRCloudRecognizeType.ACR_OPT_REC_BOTH:
